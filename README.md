...
```
	<dependencies>
		<dependency>
			...
			<version>${atomsql.version}</version>
		</dependency>
		<dependency>
			<groupId>io.github.tezch</groupId>
...
```
	<dependencies>
		<dependency>
			<groupId>io.github.tezch</groupId>
			<artifactId>atom-sql-spring-boot-starter</artifactId>
			<version>${atomsql.version}</version>
		</dependency>
		<dependency>
			<groupId>io.github.tezch</groupId>
...
メソッドに`@SqlFile`アノテーションを付与し、作成したSQLファイルを、SqlProxyインターフェイスと同じクラスパスに配置、そのファイル名を  
...
とする  
または`@SqlFile`アノテーションの値として指定することも可能  
...
ただし、アノテーションのテキストには65535byteまでという制限があるので、これより長いSQL文を使用したい場合はファイルに格納したSQL文を使用すること  
...
### 開発中のSQLログ出力  
`io.github.tezch.atomsql.Configuration#enableLog()`を`true`として起動した場合、出力したSQL文がログにINFOレベルとして出力されるようになる  
...
- __@Sensitive__  
SqlProxyのSQL実施メソッドに`@Sensitive`アノテーションを付与することで、Atom SQLが出力するログにプレースホルダバインド後のSQL文が出力されないようになる  
`@Sensitive`のみの付与の場合、全ての値が出力されなくなるが、もっと細やかに制御したい場合、アノテーションにプレースホルダ名を指定することで、指定された値のみ出力されないようにすることも可能  
...
例として、Spring Frameworkでの使用方法を例示する  
